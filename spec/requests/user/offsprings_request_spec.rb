  ...
  context "when user authenticated" do
    ...
    describe "GET #index" do
      it "returns list of offsprings" do
        ...
        offspring
        offspring2
        get user_content_offsprings_path(user)
    ...
    describe "POST #create" do
      it "creates valid offspring when valid" do
        expect { post user_content_offsprings_path(user), params: { offspring: new_offspring } }
    ...
    describe "GET #edit" do
      it "returns edit properly" do
        get edit_user_content_offspring_path(offspring)
        expect(response).to have_http_status(:success)
        expect(response.body).to include ERB::Util.html_escape offspring.first_name
      ...
      it "does not find other user's offspring" do
        get edit_user_content_offspring_path(other_offspring)
        expect(response).to redirect_to(user_offsprings_path)
    ...
    describe "GET #show" do
      it "returns offpspring properly" do
        get user_content_offspring_path(offspring)
        expect(response).to have_http_status(:success)
        expect(response.body).to include ERB::Util.html_escape offspring.first_name
        expect(response.body).to include ERB::Util.html_escape offspring.last_name
      end
      ...
      it "does not find other user's offspring" do
        get user_content_offspring_path(other_offspring)
        expect(response).to redirect_to(user_offsprings_path)
    ...
    describe "GET #update" do
      it "updates the offspring" do
        offspring
        expect do
          patch user_content_offspring_path(offspring), params: { offspring: new_offspring }
          offspring.reload
        end.to change { offspring.first_name }.to new_offspring[:first_name]
        expect(response).to redirect_to user_content_offspring_path(offspring)
      end

      it "does not update other user's offspring" do
        offspring
        expect do
          patch user_content_offspring_path(other_offspring), params: { offspring: new_offspring }
        ...
        expect(response).to redirect_to user_content_offsprings_path
      end
    ...
    describe "GET #destroy" do
      it "deletes the offspring" do
        offspring
        expect { delete user_content_offspring_path(offspring) }.to change(Offspring, :count).by(-1)
      ...
      it "does not delete other user's offspring" do
        offspring
        other_offspring
        expect { delete user_content_offspring_path(other_offspring) }.to_not change(Offspring, :count)
      ...
      it "shows a flash error if necessary" do
        ...
        expect { delete user_content_offspring_path(offspring) }.to_not change(Offspring, :count)
